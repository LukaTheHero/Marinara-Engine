...
export function CharactersPanel() {
  ...
  // Which character group folders are expanded lives in the UI store so the
  ...
  const handleDeleteGroup = useCallback(
    async (group: ParsedGroupRow) => {
      ...
      if (expandedCharacterGroupIds.includes(group.id)) toggleCharacterGroupExpanded(group.id);
    },
    [deleteGroup, expandedCharacterGroupIds, toggleCharacterGroupExpanded],
  );
    ...
    >
      ...
      <div className="flex flex-col gap-0.5">
        {parsedGroups.map((group) => {
          ...
          const isExpanded = (folderFilterActive && folderMemberIds.length > 0) || expandedCharacterGroupIds.includes(group.id);
              ...
              <div
                ...
                onClick={(event) =>
                  handleFolderRenameGesture(group.id, event, {
                    onSingleClick: () => toggleCharacterGroupExpanded(group.id),
                    onRename: () => {
